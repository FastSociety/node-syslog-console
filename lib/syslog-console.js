    ...
    SyslogConsole.MONTAGE   = null;
    SyslogConsole.BUILD     = null;
    SyslogConsole.BUILDTYPE = null;
    SyslogConsole.QUEUETIME = null;
    ...
    SyslogConsole.prototype.resetThreadHash = function() {
        SyslogConsole.THREAD = null;
    };

    SyslogConsole.prototype.resetParentHash = function() {
        SyslogConsole.PARENT = null;
    };

    SyslogConsole.prototype.resetUserId = function() {
        SyslogConsole.USER = null;
    };

    SyslogConsole.prototype.resetMontageId = function() {
        SyslogConsole.MONTAGE = null;
    };

    SyslogConsole.prototype.resetBuildId = function() {
        SyslogConsole.BUILD = null;
    };

    SyslogConsole.prototype.resetQueueTime = function() {
        SyslogConsole.QUEUETIME = null;
    };
    ...
    SyslogConsole.prototype.setMontageId = function(iMontageId) {
        SyslogConsole.MONTAGE = iMontageId;
    };

    SyslogConsole.prototype.setBuildId = function(iBuildId) {
        SyslogConsole.BUILD = iBuildId;
    };
    ...
    SyslogConsole.prototype.setQueueTime = function(iQueueTime) {
        SyslogConsole.QUEUETIME = iQueueTime;
    };

    SyslogConsole.prototype.resetHashes = function() {
        this.resetThreadHash();
        this.resetParentHash();
        this.resetRequestHash();
        this.resetUserId();
        this.resetMontageId();
        this.resetBuildId();
    };
     ...
     *
     * @param {Object}  oData
     * @param {Boolean} bRemoveThem - Remove hashes from data object, while adding them to local vars
     */
    SyslogConsole.prototype.setHashes = function(oData, bRemoveThem) {
        bRemoveThem = bRemoveThem !== undefined ? bRemoveThem : false;

        if (oData.__q)          { this.setQueueTime( oData.__q);        if (bRemoveThem) { delete oData.__q         } }
        if (oData.__t)          { this.setThreadHash(oData.__t);        if (bRemoveThem) { delete oData.__t         } }
        if (oData.__p)          { this.setParentHash(oData.__p);        if (bRemoveThem) { delete oData.__p         } }
        if (oData.__u)          { this.setUserId(    oData.__u);        if (bRemoveThem) { delete oData.__u         } }
        if (oData.__m)          { this.setMontageId( oData.__m);        if (bRemoveThem) { delete oData.__m         } }
        if (oData.__b)          { this.setBuildId(   oData.__b);        if (bRemoveThem) { delete oData.__b         } }
        if (oData.buildType)    { this.setBuildType( oData.buildType);  if (bRemoveThem) { delete oData.buildType   } }
    };
     ...
     * @param {Object}  oData
     */
    SyslogConsole.prototype.getHashesForChildRequest = function(oData) {
        this.getHashesForRequest(oData);

        if (oData.__r !== undefined) {
            delete oData.__r;
        }

        var sRequest = this.getRequestHash();
        if (sRequest !== null) {
            oData.__p = sRequest;
        }
    };
     ...
     * @param {Object}  oData
     */
    SyslogConsole.prototype.getHashesForRequest = function(oData) {
        var sThread = this.getThreadHash();
        if (sThread !== null) {
            oData.__t = sThread;
        }

        var sParent = this.getParentHash();
        if (sParent !== null) {
            oData.__p = sParent;
        }
        ...
        var iUserId = this.getUserId();
        if (iUserId !== null) {
            oData.__u = iUserId;
        }

        var iMontageId = this.getMontageId();
        if (iMontageId !== null) {
            oData.__m = iMontageId;
        }

        var iBuildId = this.getBuildId();
        if (iBuildId !== null) {
            oData.__b = iBuildId;
        }
        ...
        if (buildType !== null) {
            oData.buildType = buildType;
        }

        oData.__q = new Date().getTime();
    };
     ...
     * @return {Integer}
     */
    SyslogConsole.prototype.getMontageId = function() {
        return SyslogConsole.MONTAGE;
    };
     ...
     * @return {Integer}
     */
    SyslogConsole.prototype.getBuildId = function() {
        return SyslogConsole.BUILD;
    };
    ...
    SyslogConsole.prototype._send = function(mMessage, iSeverity, bCheckForErrors) {
        ...
        if (mMessage instanceof Object) {
            ...
            var sThread = this.getThreadHash();
            if (sThread !== null) {
                mMessage.__t = sThread;
            }

            var sParent = this.getParentHash();
            if (sParent !== null) {
                mMessage.__p = sParent;
            }

            var sRequest = this.getRequestHash();
            if (sRequest !== null) {
                mMessage.__r = sRequest;
            }

            var iUserId = this.getUserId();
            if (iUserId !== null) {
                mMessage.__u = iUserId;
            }

            var iMontageId = this.getMontageId();
            if (iMontageId !== null) {
                mMessage.__m = iMontageId;
            }

            var iBuildId = this.getBuildId();
            if (iBuildId !== null) {
                mMessage.__b = iBuildId;
            }
    ...
    SyslogConsole.prototype.timeStop = function(sLabel, mMessage, iSeverity) {
        ...
        var iDuration = 0;
        if (this.oStarts[sLabel] !== undefined) {
            this.oTimers[sLabel] = iDuration = Date.now() - this.oStarts[sLabel];
            if (mMessage instanceof Object) {
    ...
    SyslogConsole.prototype.processStart = function(sAction, oExtra) {
        ...
        if (SyslogConsole.QUEUETIME) {
            oData.queued = Date.now() - SyslogConsole.QUEUETIME;

            this.oStarts['Queued'] = SyslogConsole.QUEUETIME;
            this.timeStop('Queued');
            this.resetQueueTime();
        }

        SyslogConsole.PROCESS_NAME = this.timeStart(sAction, oData, this.oClient.LOG_NOTICE);
